...
const Box = ({ children, size, style }) => {
  ...
  className = size == "medium" ? "box box--medium" : className;
  className = size == "large" ? "box box--large" : className;

  return <div className={className} style={{...style, fontStyle:"italic"}}>{children}</div>
}
